...
create table activity_log (id int8 not null, created_at timestamp, message varchar(255), message_type varchar(255), sub_message varchar(255), shipment_id int8, user_id varchar(255), primary key (id));
...
create table app_user (id varchar(255) not null, address varchar(255) not null, city varchar(255) not null, email varchar(255) not null, enabled boolean, first_name varchar(255) not null, last_name varchar(255) not null, locked boolean, password varchar(255) not null, phone_number varchar(255) not null, role varchar(255) not null, state varchar(255) not null, default_source_address_id int8, primary key (id));
...
create table shipment (id int8 not null, created_at timestamp, delivery_date timestamp, easypost_shipment_id varchar(255) not null, easypost_status varchar(255), insurance_amount numeric(19, 2), is_insured boolean not null, postage_label_url varchar(255), public_tracking_url varchar(255), shipment_number int4 not null, status varchar(255) not null, tracking_code varchar(255), updated_at timestamp, package_id int8, rate_id int8, user_id varchar(255), primary key (id));
...
create table subscription_detail (user_id varchar(255) not null, card_last_four_digits varchar(255), card_type varchar(255), current_limit int4 not null, end_date int8, last_payment_date int8, start_date int8, status varchar(255), stripe_customer_id varchar(255), subscription_id varchar(255), subscription_plan_id varchar(255), primary key (user_id));
...
create table webhook (id int8 not null, type varchar(255), webhook_id varchar(255), primary key (id));
alter table if exists app_user add constraint UNIQUE_app_user_email unique (email);
alter table if exists person add constraint UNIQUE_person_shipment_id_type unique (shipment_id, type);
alter table if exists refresh_token add constraint UNIQUE_refresh_token unique (refresh_token);
alter table if exists shipment_address add constraint UNIQUE_shipment_address_shipment_id_type unique (shipment_id, type);
alter table if exists verification_token add constraint UNIQUE_verification_token_user_id_token_type unique (user_id, token_type);
alter table if exists webhook add constraint UNIQUE_webhook_type unique (type);
alter table if exists activity_log add constraint FK_activity_log_shipment foreign key (shipment_id) references shipment;
alter table if exists activity_log add constraint FK_activity_log_user foreign key (user_id) references app_user;
alter table if exists address add constraint FK_address_user foreign key (user_id) references app_user;
alter table if exists app_user add constraint FK_app_user_default_address foreign key (default_source_address_id) references address;
alter table if exists package add constraint FK_package_user foreign key (user_id) references app_user;
alter table if exists person add constraint FK_person_shipment foreign key (shipment_id) references shipment;
alter table if exists refresh_token add constraint FK_refresh_token_user foreign key (user_id) references app_user;
...
alter table if exists shipment add constraint FK_shipment_user foreign key (user_id) references app_user;
...
alter table if exists subscription_detail add constraint FK_subscription_detail_plan foreign key (subscription_plan_id) references subscription_plan;
alter table if exists subscription_detail add constraint FK_subscription_detail_user foreign key (user_id) references app_user;
alter table if exists verification_token add constraint FK_verification_token_user foreign key (user_id) references app_user;
